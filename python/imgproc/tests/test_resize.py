...
class TestResizeBilinear(unittest.TestCase):
    ...
    def run_resize_test(self, description, input_shape, output_shape):
        img = create_test_image(*input_shape)
        resizer = Resize(ResizeMethod.LINEAR)
        resized = resizer.resize(img, *output_shape)
...
class TestResizeBicubic(unittest.TestCase):

    def run_resize_test(self, description, input_shape, output_shape):
        img = create_test_image(*input_shape)
        resizer = Resize(ResizeMethod.CUBE)
        resized = resizer.resize(img, *output_shape)
...
class TestResizeInterArea(unittest.TestCase):

    def run_resize_test(self, description, input_shape, output_shape):
        img = create_test_image(*input_shape)
        resizer = Resize(ResizeMethod.AREA)
        resized = resizer.resize(img, *output_shape)
